...
const EventTypeTable = () => {
  ...
  useEffect(() => {
    const fetchEventTypes = async () => {
      setIsLoading(true);
      try {
        console.log("ingresa al fetchEventTypes");
        const response = await Api.get('/event-types'); // Ajusta el endpoint según sea necesario
        console.log("Luego de la petición a la API");
        if (response.statusCode === 200) {
          setEventTypes(response.data);
        } else {
          showAlert('Error', 'No se pudieron cargar los tipos de evento', 'error');
  ...
  const columns = [
    ...
    },
    {
      ...
      selector: row => row.name_event_type,
      sortable: true,
      cell: row => (
        <button
          className="text-blue-500 hover:underline text-left"
          onClick={() => handleRowClick(row._id)} // Redirige al detalle del tipo de evento
        >
          {row.name_event_type}
        </button>
      ),
  ...
  const handleRowClick = (id) => {
    navigate(`/event-types/${id}`); // Redirige al componente de detalles
  };

  return (
    <div className="p-6 bg-white shadow-md rounded-lg">
      <div className="flex justify-between items-center mb-4">
        <h2 className="text-2xl font-bold">Lista de Tipos de Evento</h2>
        <button
