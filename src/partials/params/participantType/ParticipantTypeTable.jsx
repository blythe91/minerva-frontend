...
const ParticipantTypeTable = () => {
  const [participantTypes, setParticipantTypes] = useState([]);
  const [filterText, setFilterText] = useState('');
  const [isLoading, setIsLoading] = useState(false);
  const navigate = useNavigate(); // Inicializa useNavigate

  // Fetch Participant Types
  useEffect(() => {
    const fetchParticipantTypes = async () => {
      setIsLoading(true);
      try {
        console.log("ingresa al fetchParticipantTypes");
        const response = await Api.get('/participant-types'); // Ajusta el endpoint según sea necesario
        console.log("Luego de la petición a la API");
        if (response.statusCode === 200) {
          setParticipantTypes(response.data);
        } else {
          showAlert('Error', 'No se pudieron cargar los tipos de participante', 'error');
  ...
  const filteredItems = participantTypes.filter(item => {
    const valuesToFilter = [
      item.name_participant_type,
      item.description_participant_type,
    ];
  ...
  const columns = [
    ...
    {
      name: 'Nombre del Tipo de Participante',
      selector: row => row.name_participant_type,
      sortable: true,
      cell: row => (
        <button
          className="text-blue-500 hover:underline text-left"
          onClick={() => handleRowClick(row._id)} // Redirige al detalle del tipo de participante
        >
    ...
    {
      name: 'Descripción',
      selector: row => row.description_participant_type,
      sortable: true,
    },
  ];
  ...
  const handleRowClick = (id) => {
    navigate(`/participant-types/${id}`); // Redirige al componente de detalles
  };

  return (
    <div className="p-6 bg-white shadow-md rounded-lg">
      <div className="flex justify-between items-center mb-4">
        <h2 className="text-2xl font-bold">Lista de Tipos de Participante</h2>
        <button
...
export default ParticipantTypeTable;
